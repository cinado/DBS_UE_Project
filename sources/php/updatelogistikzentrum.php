...
$id = $name = $ort = $plz = $strasse = $hausnummer = '';
...
<body>
    ...
    <div class="container">
        <div class="row mt-2 justify-content-center">
            <form action="updatelogistikzentrum.php" method="post"
    ...
    if (!$db->isEmpty($id) && (!($db->isEmpty($ort)) || !($db->isEmpty($plz)) || !($db->isEmpty($name)) || !($db->isEmpty($hausnummer)) || !($db->isEmpty($strasse)))) {
        $logistikzentrum = $db->updateLogistikzentrum($id, $name, $ort, $plz, $strasse, $hausnummer);
        if (!empty($logistikzentrum)) { ?>
            ...
            echo "Error can't update Logistikzentrum '{$id} {$name} {$ort} {$plz} {$strasse} {$hausnummer}'!";
        }
    }
    ?>
</body>
